package imanolpc.kardia.core.anki
...
class AnkiDroidConnector(private val context: Context) {
    ...
    suspend fun addNotesToAnkiDroid(
        ...
        for (note in notes) {
            ...
            val fields = arrayOf(note.front, note.back, note.sourceText)
            val tags = note.tags
    ...
    private fun findOrCreateModel(api: AddContentApi): Long? {
        ...
        return try {
            ...
            existingModelId ?: api.addNewCustomModel(
                modelName,
                arrayOf("Front", "Back", "Source"), // Campos
                arrayOf("Card 1"),        // Nombre de plantilla
                arrayOf("{{Front}}"),     // QFmt
                arrayOf("{{Front}}<br><hr id=answer>{{Back}}{{#Source}}<br><br><details><summary style='font-size:16px;color:#3f51b5;cursor:pointer;'>Origen del texto</summary><div style='font-size:15px;color:gray;text-align:left;padding:8px;background:#f5f5f5;border-radius:4px;'>{{Source}}</div></details>{{/Source}}"), // AFmt
