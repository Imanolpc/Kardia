...
class AnkiApkgCompiler(private val context: Context) {
    ...
    suspend fun compile(
        ...
        return@withContext try {
            ...
            cards.forEachIndexed { index, card ->
                ...
                val flds = "${card.front}$ANKI_FIELD_SEPARATOR${card.back}$ANKI_FIELD_SEPARATOR${card.sourceText}"
                val sfld = card.front
    ...
    private fun buildModelsJson(modelId: Long, creationTime: Long): String {
        ...
        val qfmt = "{{Front}}"
        val afmt = "{{Front}}\\n\\n<hr id=answer>\\n\\n{{Back}}\\n\\n{{#Source}}<br><br><details><summary style='font-size:16px;color:#3f51b5;cursor:pointer;'>Origen del texto</summary><div style='font-size:15px;color:gray;text-align:left;padding:8px;background:#f5f5f5;border-radius:4px;'>{{Source}}</div></details>{{/Source}}"
